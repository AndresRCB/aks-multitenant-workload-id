resource "kubernetes_deployment" "subscriberten2" {
  metadata {
    name      = "subscriberten2"
    labels    = {
      app                           = "subscriberten2"
    ...
    template {
      ...
      spec {
        ...
        container {
          ...
          env {
            name  = "SPRING_CLOUD_STREAM_BINDERS_EVENTHUB_1_ENVIRONMENT_SPRING_CLOUD_AZURE_EVENTHUBS_CONSUMER_CONSUMER_GROUP"
            value = "$Default"
          }
          env {
